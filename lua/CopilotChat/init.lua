...
local function blend_color_with_neovim_bg(color_name, blend)
    ...
    if not color_int or not bg_int then return end

    local color = {
        (color_int / 65536) % 256, (color_int / 256) % 256, color_int % 256
    }
...
local function find_lines_between_separator(lines, pattern, at_least_one)
    ...
    for i = line_count, 1, -1 do -- Reverse the loop to start from the end
        ...
        if string.find(line, pattern) then
            if i < (separator_line_finish + 1) and
                (not at_least_one or found_one) then
    ...
    if at_least_one and not found_one then return {}, 1, 1, 0 end
...
local function update_prompts(prompt, system_prompt)
    ...
    if try_again then return update_prompts(result, system_prompt) end
...
local function append(str)
    ...
    if M.config.auto_follow_cursor then state.chat:follow() end
end

local function complete()
    local line = vim.api.nvim_get_current_line()
    local col = vim.api.nvim_win_get_cursor(0)[2]
    if col == 0 or #line == 0 then return end

    local prefix, cmp_start = unpack(vim.fn.matchstrpos(line:sub(1, col),
                                                        [[\(/\|@\)\k*$]]))
...
local function get_selection()
    ...
    if state.config and state.config.selection and
        vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_win_is_valid(winnr) then
...
local function key_to_info(name, key)
    ...
    if key.detail and key.detail ~= '' then out = out .. '. ' .. key.detail end
...
function M.prompts(skip_system)
    ...
    for name, prompt in pairs(M.config.prompts) do
        ...
        if type(prompt) == 'string' then
            val = {prompt = prompt, kind = get_prompt_kind(name)}
...
function M.highlight_selection(clear)
    ...
    if not selection.start_row or not selection.end_row then return end
    vim.api.nvim_buf_set_extmark(state.source.bufnr, selection_ns,
...
function M.open(config, source, no_insert)
    ...
    local should_reset = state.config and
                             not utils.table_equals(config.window,
                                                    state.config.window)
    state.config = config
    ...
    if should_reset then M.close() end

    state.chat:open(config)
    state.chat:focus()
    state.chat:follow()

    if not no_insert and not state.copilot:running() and
        M.config.auto_insert_mode and state.chat:active() then
        vim.cmd('startinsert')
    end
end
...
--- @returns string
function M.response() return state.response end

--- Select a Copilot GPT model.
function M.select_model()
    state.copilot:select_model(function(models)
        vim.schedule(function()
            vim.ui.select(models, {prompt = 'Select a model'},
                          function(choice) M.config.model = choice end)
        end)
    end)
end
...
function M.ask(prompt, config, source)
    config = vim.tbl_deep_extend('force', M.config, config or {})
    prompt = prompt or ''
    local system_prompt, updated_prompt =
        update_prompts(prompt, config.system_prompt)
    ...
    if config.clear_chat_on_new_prompt then M.stop(true, true) end

    state.last_system_prompt = system_prompt
    local selection = get_selection()
    local filetype = selection.filetype or
                         (vim.api.nvim_buf_is_valid(state.source.bufnr) and
                             vim.bo[state.source.bufnr].filetype)
    local filename = selection.filename or
                         (vim.api.nvim_buf_is_valid(state.source.bufnr) and
                             vim.api.nvim_buf_get_name(state.source.bufnr))
    ...
    local function on_error(err)
        vim.schedule(function()
            ...
            append('\n\n' .. config.question_header .. config.separator ..
                       '\n\n')
            state.chat:finish()
            if M.config.auto_follow_cursor and M.config.auto_insert_mode and
                state.chat:active() then vim.cmd('startinsert') end
        end)
    end
            ...
            state.copilot:ask(updated_prompt, {
                ...
                on_done = function(response, token_count)
                    vim.schedule(function()
                        ...
                        if tiktoken.available() and token_count and token_count >
                            0 then
                        ...
                        if config.auto_follow_cursor and config.auto_insert_mode and
                            state.chat:active() then
                            vim.cmd('startinsert')
                        end
                    end)
                end,
                on_progress = function(token)
                    vim.schedule(function() append(token) end)
                end
            })
        end
    })
...
function M.stop(reset, no_insert)
    ...
    if not stopped then wrap = function(fn) fn() end end
...
function M.reset() M.stop(true) end
...
function M.save(name, history_path)
    ...
    if history_path then state.copilot:save(name, history_path) end
end
...
function M.debug(debug)
    ...
    local logfile = string.format('%s/%s.log', vim.fn.stdpath('state'),
                                  plugin_name)
    log.new({
        plugin = plugin_name,
...
function M.setup(config)
    ...
    if config and config.mappings then
        for name, key in pairs(config.mappings) do
            if type(key) == 'string' then
                vim.notify('config.mappings.' .. name ..
    ...
    if found_old_format then config.mappings = nil end

    M.config = vim.tbl_deep_extend('force', default_config, config or {})
    if M.config.model == 'gpt-4o' then M.config.model = 'gpt-4o-2024-05-13' end
    ...
    vim.api.nvim_set_hl(hl_ns, '@diff.plus',
                        {bg = blend_color_with_neovim_bg('DiffAdd', 20)})
    vim.api.nvim_set_hl(hl_ns, '@diff.minus',
                        {bg = blend_color_with_neovim_bg('DiffDelete', 20)})
    vim.api.nvim_set_hl(hl_ns, '@diff.delta',
                        {bg = blend_color_with_neovim_bg('DiffChange', 20)})
    vim.api.nvim_set_hl(0, 'CopilotChatSpinner',
                        {link = 'CursorColumn', default = true})
    vim.api.nvim_set_hl(0, 'CopilotChatHelp',
                        {link = 'DiagnosticInfo', default = true})
    vim.api.nvim_set_hl(0, 'CopilotChatSelection',
    ...
    if state.diff then state.diff:delete() end
    state.diff = Overlay('copilot-diff', hl_ns, diff_help, function(bufnr)
        ...
        map_key(M.config.mappings.accept_diff, bufnr, function()
            ...
            if #lines > 0 then
                vim.api.nvim_buf_set_text(state.source.bufnr,
                                          selection.start_row - 1,
                                          selection.start_col - 1,
                                          selection.end_row - 1,
                                          selection.end_col, lines)
            end
        end)
    end)

    if state.system_prompt then state.system_prompt:delete() end
    state.system_prompt = Overlay('copilot-system-prompt', hl_ns, overlay_help,
                                  function(bufnr)
        map_key(M.config.mappings.close, bufnr, function()
    ...
    if state.user_selection then state.user_selection:delete() end
    state.user_selection = Overlay('copilot-user-selection', hl_ns,
                                   overlay_help, function(bufnr)
        map_key(M.config.mappings.close, bufnr, function()
    ...
    state.chat = Chat(chat_help, function(bufnr)
        ...
        map_key(M.config.mappings.accept_diff, bufnr, function()
            ...
            local chat_lines = vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)
            local section_lines = find_lines_between_separator(chat_lines,
                                                               M.config
                                                                   .separator ..
                                                                   '$', true)
            local lines = find_lines_between_separator(section_lines,
                                                       '^```%w*$', true)
            if #lines > 0 then
                vim.api.nvim_buf_set_text(state.source.bufnr,
                                          selection.start_row - 1,
        ...
        map_key(M.config.mappings.show_diff, bufnr, function()
            ...
            local chat_lines = vim.api.nvim_buf_get_lines(state.chat.bufnr, 0,
                                                          -1, false)
            local section_lines = find_lines_between_separator(chat_lines,
                                                               M.config
                                                                   .separator ..
                                                                   '$', true)
            local lines = table.concat(find_lines_between_separator(
        ...
        map_key(M.config.mappings.show_system_prompt, bufnr, function()
            ...
            state.system_prompt:show(prompt, 'markdown', 'markdown',
                                     state.chat.winnr)
        end)
    ...
    for name, prompt in pairs(M.prompts(true)) do
        vim.api.nvim_create_user_command('CopilotChat' .. name, function(args)
            ...
            if input then M.ask(input, prompt) end
        end, {
            nargs = '*',
            force = true,
            range = true,
        ...
        if prompt.mapping then
            vim.keymap.set({'n', 'v'}, prompt.mapping,
                           function() M.ask(prompt.prompt, prompt) end, {
                desc = prompt.description or (plugin_name .. ' ' .. name)
            })
        end
    end
    ...
    vim.api.nvim_create_user_command('CopilotChat', function(opts)
        ...
        -- Call M.ask with the filtered arguments
        M.ask(filtered_args_str)
    end, {nargs = '*', force = true, range = true})

    vim.api.nvim_create_user_command('CopilotChatModels',
                                     function() M.select_model() end,
                                     {force = true})

    vim.api.nvim_create_user_command('CopilotChatOpen', function() M.open() end,
                                     {force = true})
    vim.api.nvim_create_user_command('CopilotChatClose',
                                     function() M.close() end, {force = true})
    vim.api.nvim_create_user_command('CopilotChatToggle',
                                     function() M.toggle() end, {force = true})
    vim.api.nvim_create_user_command('CopilotChatStop', function() M.stop() end,
                                     {force = true})
    vim.api.nvim_create_user_command('CopilotChatReset',
                                     function() M.reset() end, {force = true})
    ...
    vim.api.nvim_create_user_command('CopilotChatSave',
                                     function(args) M.save(args.args) end, {
        nargs = '*',
        force = true,
        complete = complete_load
    })
    vim.api.nvim_create_user_command('CopilotChatLoad',
                                     function(args) M.load(args.args) end, {
        nargs = '*',
        force = true,
        complete = complete_load
    })
